...
$sql1 = "SELECT * FROM complaints_detail WHERE status IN (7,10,11,14,15,17) AND faculty_id = '$faculty_id'";
...
<body>
                                            ...
                                            <div class="d-flex flex-column align-items-center">
                                                <div class="circle-container">
                                                    ...
                                                    <div class="circle circle3">
                                                        <div style="padding-top: 20px;">
                                                            <h1 class="font-light text-white">
                                                                <?php $query3 = "SELECT COUNT(*) as progress FROM complaints_detail WHERE status IN (7,10,11,14,15,17) AND faculty_id = '$faculty_id'";
                                            ...
                                            <div class="modal-body">
                                                <form id="add_feedback">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <!-- Reassign Button -->
                                                        <button type="button" class="btn btn-daanger" id="reassignBtn">Reassign</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (res.status === 200) {
                            ...
                            alert(res.message);
                            $('#user').load(location.href + " #user"); // Optional: refresh the page to reflect changes
                        } else {
        ...
        $(document).on('click', '.statusBtn', function() {
            ...
            $.ajax({
                type: "POST",
                url: "backend.php", // Make sure this is the correct backend URL
                data: {
        ...
        $(document).on('click', '.feedbackBtn', function() {
            ...
            $('#feedback_id').val(id); // Set id in the hidden input
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 200) {
                        ...
                        $('#completedTable').load(location.href + " #completedTable"); // Close modal on success
                    } else {
        ...
        $('#reassignBtn').on('click', function(e) {
            ...
            var id = $('#feedback_id').val(); // Get the id
            ...
            $.ajax({
                ...
                data: {
                    id: id,
                    action: 'reassign'
                }, // Pass action to indicate reassignment
                dataType: "json",
                success: function(response) {
                    if (response.status == 200) {
                        alert(response.message);
                        $('#feedback_modal').modal('hide');
                        $('#ProgressTable').load(location.href + " #ProgressTable"); // Close modal on success
                        $('#user').load(location.href + " #user");
                    } else {
                        alert(response.message || 'An error occurred while reassigning.');
                    }
                },
                error: function(xhr, status, error) {
                    console.error("AJAX Error: ", xhr.responseText);
                    alert('An error occurred while reassigning: ' + error);
                }
            });
        });
    </script>
